  ...
  return (
    <div className="iphone-screen select-none bg-ink">
      ...
      <main className="relative w-full flex-1 min-h-0 overflow-hidden">
        ...
        {/* Route chip — clean single pill, native style */}
        <div
          ref={bubbleRef}
          key={customerIndex}
          className="absolute top-[13%] left-1/2 -translate-x-1/2 z-20 bg-surface-raised border border-line rounded-full shadow-card px-4 py-2 whitespace-nowrap"
        >
          <span className="text-sm font-semibold text-text-primary">{customer.distance}</span>
          <span className="text-text-muted text-sm mx-2">·</span>
          <span className="text-sm text-text-secondary">{customer.time} away</span>
        </div>
